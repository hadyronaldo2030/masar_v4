...
@section('style')
<link href="{{asset($globalVariable .'assetsManeger')}}/css/editProject.css" rel="stylesheet"">
@endsection
...
@section('content')
    ...
    <header>
        ...
        <form class="popupWindow" method="post" action="{{ route('manager.marketing.updatetask',$project->id) }}" enctype="multipart/form-data">
            ...
            <div class="modal-body row">
                <div class="boxform col-9 mb-3">
                    ...
                    <div class="input-group mb-4">
                        <input class="form-control" type="text" id="username" name="project_name" value="{{ $project->project_name }}" required>
                    ...
                    <div class="d-flex mb-4">
                        <div class="input-group">
                            <input class="form-control text-center" type="date" id="start_date" name="start_date"  value="{{ $project->start_date }}" required>
                        ...
                        <div class="input-group  ml-4">
                            <input class="form-control text-center" type="date" id="end_date" name="end_date"  value="{{ $project->end_date }}" required>
                    ...
                    <div class="input-group  mb-4">
                        <select class="form-select"  name="department">
                            <option value="" {{ $project->department == '' ? 'selected' : '' }}>Type Department</option>
                            <option value="marketing" {{ $project->department == 'marketing' ? 'selected' : '' }}>Marketing</option>
                            <option value="video" {{ $project->department == 'video' ? 'selected' : '' }}>Video</option>
                            <option value="graphic" {{ $project->department == 'graphic' ? 'selected' : '' }}>Graphic</option>
                            <option value="administrative" {{ $project->department == 'administrative' ? 'selected' : '' }}>Administrative</option>
                            <option value="photography" {{ $project->department == 'photography' ? 'selected' : '' }}>Photography</option>
                    ...
                    <div class="input-group mb-4">
						...
						<select id="tmLeader"  class="form-select js-example-tags" name="tmLeader"  style="width: 100%;" required>
                            ...
                            {{-- <option value="{{ $project->tmLeader}}">{{ $project->tmLeader}}</option> --}}
                            @if ($project->tmLeader)
                            @php
                                $teamLeader = \App\Models\User::find($project->tmLeader);
                            @endphp
                            @if ($teamLeader)
                    ...
                    <div class="input-group">
						...
						<select id="team"  class="form-select js-example-tags" multiple="multiple" name="team[]"  style="width: 100%;" required>
                            ...
                            @php
                                $teamMembers = \App\Models\User::whereIn('id', json_decode($project->team))->get();
                            @endphp
                            @foreach ($teamMembers as $teamMember)
                                <option value="{{ $teamMember->id }}" selected>{{ $teamMember->name }}</option>
                            @endforeach
                        @endif
						</select>
                    </div>
                ...
                <div class="col mb-3">
                    <div class="input-group uploadImg">
                        <span class="minImg">
                            <label for="file-upload22" class="custom-upload-button" style="z-index: 1">Upload Img or PDF</label>
                            <input type="file" id="file-upload22"   name="file_marketing"   class="file-upload">
                            @switch(pathinfo($project->file_marketing, PATHINFO_EXTENSION) == 'pdf')
                                @case('pdf')
                                <img id="preview-image" src="{{ asset('assets/img/pdf.png') }}" alt="Uploaded Image">
                                    @break
                                @default
                                <img id="preview-image" src="{{  asset($globalVariable . 'images' ) }}/manager/marketing/images/{{  $project->file_marketing }}" alt="Uploaded Image">
                            @endswitch                 
               ...
               @enderror
                ...
                <div class="input-group">
                    <textarea class="form-control" placeholder="Type Notes" name="notesmar" aria-label="With textarea">{{ $project->notesmar }}</textarea>
                </div>
                @error('notesmar')
                    <div class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="input-group my-4">
                    <button type="submit" class="btn btnGray py-2" id="">Save changes</button>
                </div>
            </div>
        </form>
