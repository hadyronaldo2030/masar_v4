...
@section('content')
    ...
    <header>
        ...
        <form class="popupWindow" method="POST" action="{{ route('manager.programming.updatetask',$projectPro->id) }}" enctype="multipart/form-data">
            ...
            <div class="modal-body row">
                <div class="boxform col-9 mb-3">
                    ...
                    <div class="input-group mb-4">
                        <input class="form-control" type="text" id="username" name="project_name" value="{{ $projectPro->project_name }}" required>
                    ...
                    <div class="d-flex mb-4">
                        <div class="input-group">
                            <input class="form-control text-center" type="date" id="start_date" name="start_date" value="{{ $projectPro->start_date }}" required>
                        ...
                        <div class="input-group  ml-4">
                            <input class="form-control text-center" type="date" id="end_date" name="end_date" value="{{ $projectPro->end_date }}" required>
                    ...
                    <div class="input-group  mb-4">
                        <select class="form-select"  name="department">
                            <option value="" {{ $projectPro->department == '' ? 'selected' : '' }}>Type Department</option>
                            <option value="website" {{ $projectPro->department == 'website' ? 'selected' : '' }}>Website</option>
                            <option value="android" {{ $projectPro->department == 'android' ? 'selected' : '' }}>Android</option>
                            <option value="ui" {{ $projectPro->department == 'ui' ? 'selected' : '' }}>Ui/Ux</option>
                            <option value="it" {{ $projectPro->department == 'it' ? 'selected' : '' }}>It</option>
                        </select>
                    </div>
                    @error('department')
                        <div class="invalid-feedback" role="alert">
                    ...
                    <div class="input-group mb-4">
						...
						<select id="tmLeader"  class="form-select js-example-tags" name="tmLeader"  style="width: 100%;" required>
                            ...
                            @if ($projectPro->tmLeader)
                            @php
                                $teamLeader = \App\Models\User::find($projectPro->tmLeader);
                            @endphp
                            @if ($teamLeader)
                    ...
                    <div class="input-group">
						...
						<select id="team"  class="form-select js-example-tags" multiple="multiple" name="team[]"  style="width: 100%;" required>
                            <option value=" ">Select Team Leader</option>
                        @if (!empty($projectPro->team))
                        @php
                                $teamMembers = \App\Models\User::whereIn('id', json_decode($projectPro->team))->get();
                        @endphp
                ...
                <div class="col mb-3">
                    <div class="input-group uploadImg">
                        <span class="minImg">
                            ...
                            @switch(pathinfo($projectPro->file_programming, PATHINFO_EXTENSION) == 'pdf')
                            @case('pdf')
                            <img id="preview-image" src="{{ asset('assets') }}/img/pdf.png" alt="Uploaded Image">
                                @break
                            @default
                            <img id="preview-image" src="{{ asset($globalVariable . 'images' ) }}/manager/programming/images/{{  $projectPro->file_programming }}" alt="Uploaded Image">
                        @endswitch                                     
               ...
               @enderror
                ...
                <div class="input-group">
                    <textarea class="form-control" placeholder="Type Notes" name="notespro" aria-label="With textarea">{{ $projectPro->notespro }}</textarea>
                </div>
                @error('notespro')
                    <div class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
